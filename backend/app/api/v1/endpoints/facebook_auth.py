...
async def facebook_callback(request: Request, db: AsyncSession = Depends(get_async_db)):
    ...
    - Redirects user to frontend with tokens (for browser flow)
    - Returns JSON (for API testing/Postman)
    """
    try:
        ...
        facebook_user_data = await facebook_oauth_service.get_facebook_user_info(
            token["access_token"]
        )

        user = await facebook_oauth_service.authenticate_or_create_user(
            facebook_user_data, token["access_token"]
        )
        ...
        # If request looks like browser flow → redirect
        if "text/html" in request.headers.get("accept", ""):
            return RedirectResponse(url=frontend_redirect)

        # Otherwise (Postman, API clients) → return JSON
        return {
            ...
            "facebook_access_token": token[
                "access_token"
            ],  # Include Facebook's access token
            "user": {
                "id": user.id,
                "email": user.email,
...
async def facebook_token_auth(
    auth_request: FacebookAuthRequest, db: AsyncSession = Depends(get_async_db)
):
    ...
    try:
        ...
        user = await facebook_oauth_service.authenticate_or_create_user(
            facebook_user_data, auth_request.access_token
        )
